...
class TestPlotHistogram:
    ...
    def test_normalize_to_hits_one(self, plotter_no_boundary, ad):
        """The normalize_to category gets count 1.0; others are relative to it."""
        cat_name = str(ad.obs[CAT_COL].astype(str).value_counts().index[0])
        p = plotter_no_boundary.plot_histogram(CAT_COL, normalize_to=cat_name)
        val = p.data.loc[p.data["category"] == cat_name, "count"].iloc[0]
        assert abs(val - 1.0) < 1e-9

    def test_normalize_to_none_keeps_raw_counts(self, plotter_no_boundary, ad):
        """Explicit normalize_to=None behaves identically to omitting it."""
        p = plotter_no_boundary.plot_histogram(CAT_COL, normalize_to=None)
        assert p.data["count"].sum() == ad.n_obs
